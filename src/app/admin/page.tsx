...
export default function AdminPage() {
    ...
    const [bannedUserSearchTerm, setBannedUserSearchTerm] = useState('');
        ...
        const handleUserSearch = async () => {
            try {
                const res = await fetch(`/api/admin/users?search=${userSearchTerm}&isBanned=false`);
                if (!res.ok) throw new Error('사용자 검색에 실패했습니다.');
                setUsers(await res.json());
            } catch (e: any) { setError(e.message); }
        };
    
        const handleBannedUserSearch = async () => {
            try {
                const res = await fetch(`/api/admin/users?search=${bannedUserSearchTerm}&isBanned=true`);
                if (!res.ok) throw new Error('차단된 사용자 검색에 실패했습니다.');
                                    ...
                                    <CardContent>
                                        <div className="flex gap-2 mb-4">
                                            <Input
                                                ...
                                                onKeyDown={(e) => e.key === 'Enter' && handleBannedUserSearch()}
                                            />
                                            <Button onClick={handleBannedUserSearch}><Search className="h-4 w-4" /></Button>
                                        </div>
                                        <div className="max-h-[600px] overflow-y-auto space-y-2 pr-2">
