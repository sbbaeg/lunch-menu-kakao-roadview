...
export async function fetchFullGoogleDetails(place: KakaoPlaceItem): Promise<KakaoPlaceItem> {
  try {
    ...
    if (!placeId) {
      console.log(`[Google API Info] for ${place.place_name}: Could not find placeId. Response:`, JSON.stringify(findPlaceData));
      return place; // 구글에서 장소를 못 찾으면 카카오 정보만 반환
    }
    ...
    const detailsData: { result?: GooglePlaceDetailsResult, status?: string, error_message?: string } = await detailsResponse.json();
    const result = detailsData.result;
    
    if (!result) {
      console.log(`[Google API Info] for ${place.place_name} (placeId: ${placeId}): Could not find details. Response:`, JSON.stringify(detailsData));
      return place; // 상세 정보 없으면 카카오 정보만 반환
    }
